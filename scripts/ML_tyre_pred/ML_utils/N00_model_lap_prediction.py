...
def validate_lap_data(input_data):
    ...
    if isinstance(input_data, str):
        try:
            ...
            print(
                f"Loaded data from {input_data}: {df.shape[0]} rows, {df.shape[1]} columns")
        except Exception as e:
        ...
        df = input_data.copy()
        print(
            f"Using provided DataFrame: {df.shape[0]} rows, {df.shape[1]} columns")
...
def add_sequential_features(df):
    ...
    for driver in df['DriverNumber'].unique():
        ...
        for stint in driver_data['Stint'].unique():
            ...
            if len(stint_data) < 2:
                print(
                    f"Skipping driver {driver}, stint {stint}: not enough laps")
                continue
    ...
    seq_cols = [col for col in result_df.columns if col.startswith(
...
def format_lap_predictions(df, predictions):
    ...
    if 'LapTime' in result_df.columns and not result_df['LapTime'].isna().all():
        result_df['PredictionError'] = result_df['PredictedLapTime'] - \
            result_df['LapTime']
    ...
    output_columns = [
        col for col in output_columns if col in result_df.columns]
...
if __name__ == "main":
    predictions_df = predict_lap_times(
