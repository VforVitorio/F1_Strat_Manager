...
> Everything needed to contribute back to F1 StratLab: the docs pipeline that publishes this site, the CI/CD workflows that release the wheel, and the conventions the project follows on every commit and pull request.
...
The repo uses [Conventional Commits](https://www.conventionalcommits.org/) so `release-please` can derive the next semver from the log. A PR title that follows the table below is enough to drive the changelog.
...
Match the prefix used in the commit:
