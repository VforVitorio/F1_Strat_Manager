...
**Known limitation, stated rather than hidden.** An overcut cannot currently win: inside one window it takes the same stop and pays the same lane as PIT_NOW, so it is that candidate minus a lap of fresh rubber. What makes a real overcut pay is the out-lap — a new set needs a lap to switch on, so worn-but-warm tyres can out-run it — and that warm-up is not modelled per lap. It is measurable, and the invariant test is kept as a failing expectation with the mechanism named.
