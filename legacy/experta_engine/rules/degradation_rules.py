...
import src.agents.base_agent as agent_setup
from experta import DefFacts, Fact, Field, KnowledgeEngine
from experta import Rule, NOT, OR, AND, AS, MATCH, TEST, EXISTS

# Try importing tire prediction module
try:
    from src.strategy.inference import tire_predictor as tdp
except ImportError:
    tdp = None
    print("Warning: tire_predictor module not available")
...
def test_with_real_data(race_data_path, models_path):
    ...
    from src.agents.base_agent import calculate_race_phase
...
def test_multiple_drivers(race_data_path, models_path, driver_numbers=None):
    ...
    from src.agents.base_agent import load_tire_predictions
    predictions = load_tire_predictions(race_data, models_path)
    ...
    for driver in driver_numbers:
        ...
        from src.agents.base_agent import transform_tire_predictions
        ...
        # Initialize engine
        from src.agents.base_agent import F1StrategyEngine, RaceStatusFact
        engine = F1DegradationRules()
        engine.reset()
